      ...
      <q-card-section class="q-pa-xs">
        <q-form @submit.prevent="getVentas">
          <div class="row">
            ...
            <div class="col-12 col-md-2 flex flex-center">
              <q-btn style="width: 150px" label="Nueva Venta" color="positive" @click="$router.push('/oasisventas/add')" no-caps icon="add_circle_outline"
                     :loading="loading"
              />
            </div>
        ...
        <q-markup-table wrap-cells dense flat bordered>
          ...
          <tr v-for="venta in ventas" :key="venta.id">
            ...
            <td>
              ...
              <span  v-for="detail in venta.oasisSalesDetalles" :key="detail.id">
...
function anular(id) {
  proxy.$q.dialog({
    ...
    loading.value = true;
    proxy.$axios.post(`/oasis-sales/${id}/anular`).then(() => {
      getVentas();
    }).finally(() => {
      loading.value = false;
    });
